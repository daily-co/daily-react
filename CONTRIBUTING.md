...
Thank you for looking into contributing to `daily-react-hooks`!  We welcome your contributions toward making [Daily](https://www.daily.co) the easiest real-time video API for developers to use in their products.
...
2. If you're still seeing the error, check to see if somebody else has [already filed the issue](https://github.com/daily-co/daily-react-hooks/issues) before creating a new one.
...
- Note that the `daily-react-hooks` repo is a public-facing mirror of an internal Daily repo, so when a pull request contribution is accepted, the pull request won't be merged in the tradtional way. But never fear! Your contributed commits _will_ appear, appropriately attributed to you, in the `daily-react-hooks` repo.
